...
export function useAuth() {
    ...
    async function registerPrf(name: string): Promise<void> {
        ...
        await storage.write(["prf"] as AuthMethod[], "auth_methods");
        setKeys(ecdh, sign);
    ...
    async function registerPassword(password: string): Promise<void> {
        ...
        await storage.storePublicKeys(ecdh, sign);
        await storage.write(salt, "pin_salt");
        await storage.write(["pin"] as AuthMethod[], "auth_methods");
        setKeys(ecdh, sign);
        await session.save(ecdh, sign);
    }
    ...
    async function authenticate(): Promise<void> {
        const credentialId = await storage.read<ArrayBuffer>(
            "webauthn_credential_id"
        );
    ...
    async function authenticateWithPassword(password: string): Promise<void> {
        checkLockout();
        const salt = await storage.read<Uint8Array<ArrayBuffer>>("pin_salt");
